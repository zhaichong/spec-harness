...
INDEPENDENT_ACTORS = ("新上下文", "新 agent", "不同模型", "人工")
STRICT_ACTORS = ("新 agent", "不同模型", "人工")
RISK_SURFACES = {
    "数据写入/删除": ("数据写入", "写入", "删除"),
    "权限/敏感数据": ("权限", "敏感"),
    "外部副作用": ("外部", "副作用"),
...
def independent_actor(value: str | None, strict: bool = False) -> bool:
    normalized = (value or "").lower()
    allowed = STRICT_ACTORS if strict else INDEPENDENT_ACTORS
    return "同一 agent" not in normalized and any(actor in normalized for actor in allowed)


def validate_review(directory: Path, spec: str, strict: bool, errors: list[str]) -> None:
    spec_values = ("审核者", "审核版本与输入", "审核输出引用", "独立性")
    for label in spec_values:
        if unresolved(value_after(spec, label)):
            errors.append(f"独立审核记录未填写：{label}")
    if not independent_actor(value_after(spec, "审核者"), strict):
    ...
    if version and version not in (value_after(review, "审核的 Spec") or ""):
        errors.append(f"独立审核文件版本不匹配：需要 {version}")
    if not independent_actor(value_after(review, "审核者"), strict):
        errors.append("独立审核文件的审核者不具备独立性")
    if "未读取起草过程" not in (value_after(review, "独立性声明") or ""):
        errors.append("独立审核文件缺少未读取起草过程的声明")
    conclusion = value_after(review, "结论") or ""
    if unresolved(conclusion) or "阻塞" in conclusion:
        errors.append("独立审核文件没有有效的非阻塞结论")
...
def validate_risk_mapping(spec: str, acs: list[tuple[str, str, str]], errors: list[str]) -> None:
    ...
    for surface, keywords in RISK_SURFACES.items():
        if not re.search(rf"^-\s*{re.escape(surface)}：\s*是\s*$", spec, re.MULTILINE):
            continue
        mapped = set().union(*(ids for kind, ids in mappings if any(word in kind for word in keywords))) if mappings else set()
        if not mapped:
...
def validate_draft(directory: Path, spec: str, errors: list[str]) -> list[tuple[str, str, str]]:
    ...
    status = (value_after(spec, "审核状态") or "").lower()
    strict = "流程档位：Strict" in spec
    claimed_independent = strict or "fresh review" in status or "人工复查" in status
...
def validate_confirmation(spec: str, errors: list[str]) -> None:
    ...
    elif version not in confirmation:
...
def validate_delivery_review(directory: Path, spec: str, errors: list[str]) -> None:
    ...
    conclusion = value_after(review, "审核结论") or ""
    if "通过" not in conclusion or any(word in conclusion for word in ("阻塞", "退回", "不通过")):

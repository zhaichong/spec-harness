...
面向 AI 编程 Agent 的风险自适应开发工作流。给出一句话需求即可开始；Agent 按 `SKILL.md` 选择 Fast 或 Spec。仅权限/敏感、外部副作用、不可逆或生产/批量才在同一条 Spec 路径上加码；普通写自己的库不加码。领域细则覆盖前端、Java 后端和前后端契约；非 Java 后端走主流程。

流程、档位、门禁、产物和校验命令以 `SKILL.md` 为准，不要在本文件另写一份。

## Spec 的确认闭环

进入 Spec 档后，先由 AI 写草案并自审。凡会影响需求、范围、规则、接口、数据、权限、风险或验收标准的问题，必须先向作者确认，不能自行假设后继续。作者答复导致修改时，更新 Spec、递增版本并复审；作者确认当前 Spec 版本后，才开始编码。这里的作者是对需求和业务规则有最终决定权的人，通常是提出需求的用户。

未加码且没有疑问时，流程就是：草案 → 自审无阻塞 → 作者确认 v1 → 编码。自审有阻塞则不能开写。加码任务仍按 `SKILL.md` 的既有规则完成独立复查，且独立复查的版本必须与作者最终确认的版本一致。
